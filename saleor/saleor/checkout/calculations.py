...
def _fetch_checkout_prices_if_expired(
    ...
    force_update: bool = False,
    database_connection_name: str = settings.DATABASE_CONNECTION_DEFAULT_NAME,
) -> Promise[tuple["CheckoutInfo", list["CheckoutLineInfo"]]]:
    ...
    if no_need_to_calculate_taxes:
        ...
        _set_checkout_base_prices(checkout, checkout_info, lines)
        return Promise.resolve(None).then(process_calculation_result)

    return (
        promise_calculate_taxes_with_error_handling(
            tax_calculation_strategy=tax_calculation_strategy,
            tax_app_identifier=tax_app_identifier,
            manager=manager,
            checkout_info=checkout_info,
...
def _call_plugin_or_tax_app(
    ...
    def recalculate_with_plugins(_):
        ...
        if checkout.tax_error:
            return Promise.reject(TaxDataError(checkout.tax_error))
...
def _get_promised_taxes_for_checkout(
    checkout_info: "CheckoutInfo",
    lines: list["CheckoutLineInfo"],
    tax_app_identifier: str | None,
    requestor: Union["App", "User", None],
    ...
    return (
        checkout_calculate_taxes.get_taxes(
            checkout_info, lines, tax_app_identifier, requestor=requestor
        )
...
def fetch_checkout_data(
    ...
    def process_refreshed_prices(
        ...
        if (
            checkout_info.checkout.price_expiration
            != previous_checkout_price_expiration
            ...
            or (
                ...
                and checkout_info.checkout.authorize_status
